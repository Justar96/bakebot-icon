...
export interface MascotTuning {
  /** The shell. Only a look past the deadzone moves it. */
  gaze: SpringTuning;
  /** The pupil, which does every glance and lags the shell. */
  pupil: SpringTuning;
  /** The jelly in flight. A low ratio here is what rings after an impact. */
  jellyFree: SpringTuning;
  /** The jelly in contact, which settles into the shape of the wall. */
  jellyContact: SpringTuning;
  /** Scales every deformation drive. 0 is a rigid eye, 1 is the default. */
  squish: number;
  ...
  blinkSpread: number;
  /** How far a look must reach, in viewBox units, before the shell answers it. */
  shellDeadzone: number;
}
...
const GAZE: SpringConfig = { stiffness: 420, damping: 34 };
const PUPIL: SpringConfig = { stiffness: 150, damping: 16 };
...
export function resolveTuning(tuning?: Partial<MascotTuning> | null): ResolvedTuning {
  ...
  return {
    ...
    pupil: spring(given.pupil, DEFAULT_TUNING.pupil),
    ...
    shellDeadzone: number(given.shellDeadzone, DEFAULT_TUNING.shellDeadzone, 0, 30),
  };
}
