 ...
 * decides how much remains past the deadzone and whether the travel region
 * allows it at all. Keeping the vocabulary here means a caller can supply its
 ...
 * vocabulary rather than to the eye because it decides how rare an excursion
 * is, not how the simulation draws one.
 */
export const LOOK_REACH = 18;

/**
 * Points of interest in view units from the icon centre. The deadzone removes
 * the shortest part of each look; the far few reach past the border and turn
 * the face all the way — behaviour the simulation arrives at, not a clip
 * played at it. `intentWeight` makes those full turns the rare ones.
 */
 ...
 *
 * Every place is close to the deadzone, so the eyes stay well inside the tile
 * and the state's pose carries the read. Drift, tremor and blinks continue,
 * which stops that pose from reading as a frozen picture — the state says what
 * the mascot is, and this says it is still alive while it is that.
 */
