 ...
 *
 * `sideEffects: false` plus a named-re-export entrypoint makes Bun prune the
 * whole module graph and emit a list of export names with nothing behind them —
 * a successful build and a broken package. `src/index.ts` explains the shape
 * that avoids it; this is the assertion that catches it coming back. */
